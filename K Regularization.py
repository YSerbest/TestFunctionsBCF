...
"""
GRADIENT DESCENT OPTIMIZATION WITH IMPROVED STABILITY
"""
# Initialization
g_fine = np.ones(K) / K  # Evenly distribute weights
a_fine = np.linspace(0.5, 2.0, K)  # Spread real parts
gamma_fine = np.linspace(0.01, 0.1, K)  # Small damping terms

initial_learning_rate = 0.001 / np.sqrt(K)  # Scale learning rate with K
decay_rate = 0.005  # Slower decay
lambda_reg = 0.0001 * K  # Increase regularization for larger K
num_iterations = 200
safe_exp_max = 30  # Reduce threshold for stability
loss_history = []
...
for iteration in range(num_iterations):
    ...
    for j in range(K):
        for i, t in enumerate(t_values):
            e_term = np.exp(np.clip(-1j * (a_fine[j] + 1j * gamma_fine[j]) * t, -safe_exp_max, safe_exp_max))
            ...
            grad_a[j] += np.imag(approx_diff * d_exp_dw * g_fine[j]).item()
            grad_gamma[j] += np.real(approx_diff * d_exp_dw * g_fine[j]).item()

    # Apply regularization
    grad_g += lambda_reg * g_fine
    grad_a += lambda_reg * a_fine
    grad_gamma += lambda_reg * gamma_fine

    # Clip gradients for stability
    grad_g = np.clip(grad_g, -0.5, 0.5)
    grad_a = np.clip(grad_a, -0.1, 0.1)
    grad_gamma = np.clip(grad_gamma, -0.01, 0.01)

    # Update parameters
    g_fine -= learning_rate * grad_g
    a_fine -= learning_rate * grad_a
    gamma_fine -= learning_rate * grad_gamma

    # Clip gamma_fine to prevent instability
    gamma_fine = np.clip(gamma_fine, -0.5, 0.5)
    ...
    loss += (lambda_reg / 2) * (
                np.sum(g_fine ** 2) + np.sum(a_fine ** 2) + np.sum(gamma_fine ** 2))  # L2 Regularization
...
plt.plot(t_values, C_t_original_values.real, label="Re[C(t)] (Original)", color='blue')
plt.plot(t_values, C_t_fitted_values.real, '--', label="Re[C(t)] (Approx)", color='red')

plt.plot(t_values, C_t_original_values.imag, label="Im[C(t)] (Original)", color='green')
plt.plot(t_values, C_t_fitted_values.imag, '--', label="Im[C(t)] (Approx)", color='orange')
