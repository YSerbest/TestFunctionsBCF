...
C_t_original_values = np.array([C_t_original(t, eta, omega_c, s, beta, hbar) for t in t_vals])
...
initial_learning_rate = 0.001 / np.sqrt(K)
decay_rate = 0.005  # Slower decay
lambda_reg = 0.01 * K # Regularization strength
num_iterations = 200
loss_history = []
...
for iteration in range(num_iterations):
    ...
    n = len(t_vals)
    loss = np.sum(
        np.abs(C_t_original_values - C_t_sum_exponentials(t_vals, *np.hstack([g_fine, a_fine, gamma_fine]))) ** 2) / n
    loss += (lambda_reg / 2) * (
...
PLOT RESULTS
"""
C_t_fitted_values = np.array([C_t_sum_exponentials(t, *np.hstack([g_fine, a_fine, gamma_fine])) for t in t_vals])

plt.figure(figsize=(10, 5))
plt.plot(t_vals, C_t_original_values.real, 'o', label="Original Re[C(t)]")
plt.plot(t_vals, C_t_fitted_values.real, '--', label="Fitted Re[C(t)]")
plt.xlabel("Time (t)")
plt.ylabel("Re[C(t)]")
plt.legend()
plt.grid()
...
plt.figure(figsize=(10, 5))
plt.plot(t_vals, C_t_original_values.imag, 'o', label="Original Im[C(t)]")
plt.plot(t_vals, C_t_fitted_values.imag, '--', label="Fitted Im[C(t)]")
plt.xlabel("Time (t)")
plt.ylabel("Im[C(t)]")
plt.legend()
plt.grid()
